...
// Configure multer for equipment photo uploads
...
router.post('/', authMiddleware, upload.array('photos', 5), async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      equipmentData.photos = req.files.map(file => `/uploads/equipment/${file.filename}`);
    }
    ...
    if (equipmentData.price && equipmentData.price > 0) {
      try {
        ...
        await payment.save();
        console.log('Payment record created for equipment purchase:', payment._id);
      } catch (paymentError) {
    ...
    try {
      ...
      console.log('Notification created for new equipment:', notification._id);
...
router.put('/:id', authMiddleware, upload.array('photos', 5), async (req, res) => {
  try {
    ...
    const updatedEquipment = {
      ...
      category: req.body.category || existingEquipment.category,
      ...
      status: req.body.status || existingEquipment.status,
    ...
    // Process uploaded photos (combine with existing photos)
    let allPhotos = [];
    ...
    if (req.body.existingPhotos) {
      try {
        ...
        console.warn('Error parsing existing photos:', err);
      }
    }
    
    // Add new uploaded photos
    if (req.files && req.files.length > 0) {
      const newPhotos = req.files.map(file => `/uploads/equipment/${file.filename}`);
      allPhotos = [...allPhotos, ...newPhotos];
    }
    
    updatedEquipment.photos = allPhotos;
    ...
    if (newPrice > 0 && newPrice !== oldPrice) {
      try {
        ...
        const payment = new Payment(paymentData);
        await payment.save();
        console.log('Payment record created/updated for equipment:', payment._id);
      } catch (paymentError) {
        console.error('Error creating payment record:', paymentError);
      }
    }
    ...
    if (newPrice !== oldPrice || updatedEquipment.name !== existingEquipment.name) {
      try {
        ...
        const notification = new GymNotification(notificationData);
        await notification.save();
        console.log('Notification created for equipment update:', notification._id);
      } catch (notificationError) {
        console.error('Error creating notification:', notificationError);
      }
    }
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // Get the equipment to delete (for photo cleanup)
    const equipmentToDelete = gym.equipment[equipmentIndex];
    ...
    if (equipmentToDelete.photos && equipmentToDelete.photos.length > 0) {
      for (const photoPath of equipmentToDelete.photos) {
        try {
          const fullPath = path.join(__dirname, '..', photoPath);
          await fs.unlink(fullPath);
        } catch (err) {
          console.warn('Could not delete photo file:', photoPath, err.message);
        }
      }
    }
...
router.post('/:gymId/equipment', authMiddleware, upload.array('photos', 5), async (req, res) => {
  try {
    const { gymId } = req.params;
    const {
      name,
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach(file => {
        photos.push(`/uploads/equipment/${file.filename}`);
      });
    }
...
router.put('/:gymId/equipment/:equipmentId', authMiddleware, upload.array('photos', 5), async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach(file => {
        newPhotos.push(`/uploads/equipment/${file.filename}`);
      });
    }

    // Combine existing photos with new ones (if no new photos, keep existing)
    const photos = newPhotos.length > 0 ? [...(existingEquipment.photos || []), ...newPhotos] : existingEquipment.photos || [];
...
router.delete('/:gymId/equipment/:equipmentId', authMiddleware, async (req, res) => {
  try {
    ...
    if (equipment.photos && equipment.photos.length > 0) {
      for (const photoPath of equipment.photos) {
        try {
          const fullPath = path.join(__dirname, '../', photoPath);
          await fs.unlink(fullPath);
        } catch (fileError) {
          console.warn('Could not delete equipment photo:', photoPath, fileError.message);
        }
      }
    }
...
router.delete('/:gymId/equipment/:equipmentId/photo', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('Could not delete equipment photo file:', photoPath, fileError.message);
    }
